...
function updateSettingValue(string $name, string $value): bool
{
    ...
    ], "name = 'curia_" . $db->escape_string($name) . "'");
...
{
    $log = \curia\getCacheValue('unique_log_events') ?? [];
...
function getUsersById(array $userIds, string $columnsCsv = '*'): array
{
    ...
    if ($idsToFetch) {
        $entries = \curia\queryResultAsArray(
            $db->simple_select('users', $columnsCsv, 'uid IN (' . \curia\getIntegerCsv($idsToFetch) . ')'),
            'uid'
        );
